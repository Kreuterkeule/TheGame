...
func _input(event):
	...
	if (Input.is_action_just_pressed("scroll_up") && self.zoom.x + ZOOM_SPEED.x <= MAX_ZOOM):
		self.zoom += ZOOM_SPEED;
	if (Input.is_action_just_pressed("scroll_down") && self.zoom.x - ZOOM_SPEED.x >= MIN_ZOOM):
		self.zoom -= ZOOM_SPEED;
